...
for num in range(1,6):
  ...
  if pd.isna(newFilm.poster_url.values[num]) == False:
    ...
    cols[num+1].image(newFilm.poster_url.values[num], width = 150)
  else:
    cols[num+1].subheader(newFilm.title.values[num])
    cols[num+1].image('https://upload.wikimedia.org/wikipedia/commons/e/e6/Pas_d%27image_disponible.svg', width = 150)
skey = st.secrets["key"]
