...
for steps in range(step_range):
  ...
  for num in range(1 + next_line,6 + next_line):
    ...
    cols[(num-1) - next_line].markdown("<p style='text-align: center'><b>{}</b>   -   {}</p>".format(newFilm.averageRating.values[num], newFilm.startYear.values[num]), unsafe_allow_html=True)
