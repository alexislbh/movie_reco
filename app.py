...
for steps in range(step_range):
  ...
  for num in range(1 + next_line,6 + next_line):
    if num == reco_val:
      break
    cols[(num-1) - next_line].markdown("<p style='text-align: center'><b>{}</b>{}<i>{}</i></p>"
                                       .format(round(newFilm.averageRating.values[num],1), newFilm[newfil['ab':'zu']== True].values[num],newFilm.startYear.values[num]), unsafe_allow_html=True)
